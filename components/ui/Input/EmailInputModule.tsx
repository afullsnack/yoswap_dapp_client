...
export const EmailInputModule: React.FC<EmailInputProps> = ({
  label,
  placeholder,
  value,
  onChange,
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Basic email format validation before verifying
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
    if (emailRegex.test(value) && onVerify) {
      setIsVerifying(true)
